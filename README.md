...
窓番号はキャラではなく窓に付く。一度番号が決まった窓は、ログアウトしても、
AccountList に無いキャラでログインしても、その番号のまま切り替え先になる。
番号が変わるのは、AccountList にある別の番号のキャラでログインしたときだけ。

まだ一度も AccountList のキャラでログインしていない窓は、切り替え先にならない。
ログイン時に警告が出る。
